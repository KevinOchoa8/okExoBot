...
import App.DesktopApp.CustomControl.okLabel;
import App.DesktopApp.CustomControl.okTextBox;
...
public class okSexo extends JPanel implements ActionListener {
    private transient FactoryBL<okSexoDTO> blFactory = new FactoryBL<>(okSexoDAO.class);
    private transient okSexoDTO dtoSexo = new okSexoDTO();
    ...
    public okSexo(){
        try {
            ...
            okAppMSG.show("Error al cargar los datos tipo de persona: " + e.getMessage());
        }
    }
    
    private void loadRowData() throws Exception {
        regAct   = blFactory.getMinReg("IdCatalogo");
        regMax   = blFactory.getMaxReg("IdCatalogo");
    ...
    private void showDataTable() throws Exception {
        ...
        int index = 0;
        for (okSexoDTO  d : blFactory.getAll()) {
    ...
    private void btnGuardarClick() {
        ...
        try {
            if (okAppMSG.showConfirmYesNo("¿Seguro que desea " + ((isDTONull) ? "AGREGAR ?" : "ACTUALIZAR ?"))) {
                if (isDTONull)
                    dtoSexo = new okSexoDTO(0, 2, txtNombre.getText().trim(), "", "A", null, null);
                else
            ...
            okAppMSG.showError( e.getMessage());
        }
    }
    private void btnEliminarClick() {
        try {
            if (okAppMSG.showConfirmYesNo("Seguro que desea Eliminar?")) {
    ...
    ************************/
    ...
    private okLabel 
            lblTitulo   = new okLabel("TIPO DE SEXO"),
            lblIdSexo   = new okLabel(" Código Sexo :"),
            lblNombre   = new okLabel("*Tipo de Sexo:"),
            lblTotalReg = new okLabel(" 0 de 0 ");
    private okTextBox 
            txtIdSexo   = new okTextBox(),
            txtNombre   = new okTextBox();
    private okButton 
            btnPageIni  = new okButton(" |< "),
            btnPageAnt  = new okButton(" << "),
            btnPageSig  = new okButton(" >> "),
            btnPageFin  = new okButton(" >| "),

            btnRowIni   = new okButton(" |< "),
            btnRowAnt   = new okButton(" << "),
            btnRowSig   = new okButton(" >> "),
            btnRowFin   = new okButton(" >| "),

            btnNuevo    = new okButton("Nuevo"),
            btnGuardar  = new okButton("Guardar"),
            btnCancelar = new okButton("Cancelar"),
            btnEliminar = new okButton("Eliminar");
    private JPanel 
            pnlTabla    = new JPanel(),
    ...
    private void initComponents() {
        ...
        pnlBtnPage.add(new okLabel(" Page:( "));
        //pnlBtnPage.add(lblTotalReg); //cambiar
        pnlBtnPage.add(new okLabel(" ) "));
        ...
        pnlBtnCRUD.setBorder(okAppStyle.createBorderRect());
